...
export default function KuisM1K10() {
    ...
    const next = () => {
        if (lock === true) {
            if (index === dataM2K10.length - 1) {
                setResult(true);
                return 0;
            }
            setIndex(prev => prev + 1);
            setQuestion(dataM2K10[index + 1]); // Update this line
            setLock(false);
                ...
                <div className=" bg-white rounded-xl py-10 px-5 mx-10 justify-center lg:w-1/2 md:w-1/3 drop-shadow-2xl shadow-2xl">
                    ...
                    {result ? <></> : <>
                        ...
                        <div className="justify-center text-center">
                            ...
                            <div className="index text-sm">{index + 1} of {dataM2K10.length} questions</div>
                        </div>
                    </>}
                    {result ? (
                        <div className="text-center">
                            <h1 className="font-bold text-8xl mb-10">{Math.round(score / dataM2K10.length * 100)}</h1>
                            <h1 className="mb-5 text-lg font-semibold">
                                You Score {score} out of {dataM2K10.length} questions
                            </h1>

...
export default function KuisM1K10() {
    ...
    const next = () => {
        if (lock === true) {
            if (index === dataM3K11.length - 1) {
                setResult(true);
                return 0;
            }
            setIndex(prev => prev + 1);
            setQuestion(dataM3K11[index + 1]); // Update this line
            setLock(false);
            ...
            <div className="w-full flex items-center justify-center">
                <div className=" bg-white rounded-xl py-10 px-5 mx-10 justify-center lg:w-1/2 md:w-1/3 drop-shadow-2xl shadow-2xl">
                    ...
                    <div className=" text-center mb-20 mx-auto"><h3 className="font-medium text-lg mb-2">Kuis Kelas 11</h3>
                        <h1 className="font-bold text-3xl">Passive Voice</h1>
                    </div>
                    {result ? <></> : <>
                        ...
                        <div className="justify-center text-center">
                            ...
                            <div className="index text-sm">{index + 1} of {dataM3K11.length} questions</div>
                        </div>
                    </>}
                    {result ? (
                        <div className="text-center">
                            <h1 className="font-bold text-8xl mb-10">{Math.round(score / dataM3K11.length * 100)}</h1>
                            <h1 className="mb-5 text-lg font-semibold">
                                You Score {score} out of {dataM3K11.length} questions
                            </h1>

...
import { DataLatGramMud } from "./DataLatGramMud";
import { postQuiz } from "../../Kuis/quizcontroler/quizpost";
import NavbarHome from "../../Navbar/NavbarHome";
import { useNavigate } from 'react-router-dom';


export default function LatihanGramMudah() {
    let [index, setIndex] = useState(0);
    let [question, setQuestion] = useState(DataLatGramMud[index]);
    ...
    const next = () => {
        if (lock === true) {
            if (index === DataLatGramMud.length - 1) {
                setResult(true);
                return 0;
            }
            setIndex(prev => prev + 1);
            setQuestion(DataLatGramMud[index + 1]); // Update this line
            setLock(false);
    ...
    const data = {
        quizname: "Latihan Grammar Mudah",
        score: score * 20,
        percobaan: 0,
                        ...
                        {result ? (
                            <div className="text-center">
                                ...
                                <h1 className="mb-5 text-lg font-semibold">
                                    You Score {score} out of {DataLatGramMud.length} questions
                                </h1>

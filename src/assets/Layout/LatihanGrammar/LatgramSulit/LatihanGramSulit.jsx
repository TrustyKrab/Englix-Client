...
import { DataLatGramSul } from "./DataLatGramSul";
import { postQuiz } from "../../Kuis/quizcontroler/quizpost";
import NavbarHome from "../../Navbar/NavbarHome";
import { useNavigate } from 'react-router-dom';


export default function LatihanGramSulit() {
    let [index, setIndex] = useState(0);
    let [question, setQuestion] = useState(DataLatGramSul[index]);
    ...
    const next = () => {
        if (lock === true) {
            if (index === DataLatGramSul.length - 1) {
                setResult(true);
                return 0;
            }
            setIndex(prev => prev + 1);
            setQuestion(DataLatGramSul[index + 1]); // Update this line
            setLock(false);
    ...
    const data = {
        quizname: "Latihan Grammar Sulit",
        score: score * 20,
        percobaan: 0,
                    ...
                    <div className=" bg-white rounded-xl py-10 px-5 mx-10 justify-center lg:w-1/3 md:w-1/4 drop-shadow-2xl shadow-2xl">
                        {result ? <></> : <>
                            ...
                            <div className="justify-center text-center">
                                ...
                                <div className="index text-sm">{index + 1} of {DataLatGramSul.length} questions</div>
                            </div>
                        </>}
                        {result ? (
                            <div className="text-center">
                                <h1 className="font-bold text-8xl mb-10">{Math.round(score / DataLatGramSul.length * 100)}</h1>
                                <h1 className="mb-5 text-lg font-semibold">
                                    You Score {score} out of {DataLatGramSul.length} questions
                                </h1>

...
export default function SidebarK10() {
    ...
    return (
        <div className="relative flex flex-col md:flex-row bg-blue-100 ">
            ...
            >
                <div className="text-lg relative h-[80vh] overflow-y-scroll scrollbar-thin px-10 py-2 pb-14 md:py-10 sm:py-14">
                    {coba.map((data, index) => (
                        <a
                            key={index}
            ...
            {/* Konten */}
            <div
                className={`flex-1 max-h-[84vh] pb-10 mt-10 relative overflow-y-auto transition-transform duration-300 ${isSidebarOpen ? 'transform translate-x-full' : 'transform translate-x-0'}`}
            >
                <button
                    className="md:hidden text-2xl -mt-4 fixed top-1 left-4 z-50 text-white rounded"
